...
export default function ConvocatoriasPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Drawer open={open} onClose={onClose} title="Convocatorias disponibles">
      <div className="stack">
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <Link className="link" to="/convocatorias" onClick={onClose}>Ver todas</Link>
        </div>
            ...
            <div key={c.id} className="card" style={{ padding: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {effectiveRole === 'admin' && (
                    <Link className="btn" to={`/admin?convocatoria=${c.id}`} onClick={onClose}>Gestionar</Link>
                  )}
                  {effectiveRole === 'evaluador' && (
                    <Link className="btn" to={`/mis-evaluaciones?convocatoria=${c.id}`} onClick={onClose}>Ver asignadas</Link>
                  )}
